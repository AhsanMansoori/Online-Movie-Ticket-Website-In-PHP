...
if(isset($_POST['RegBtn']))
{
    if(!empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['confirm_password']))
    {
        ...
        {
            $subject = "Welcome ";
             $txt = "Thanks to join MovieJet Community! We Will notify you every upcomming Movies!";
...
<body>
    ...
    if($check)
    {
      ...
      YOu have been registered successfully! Try to login Now :) 
    </div>";
    }
    if($check2)
    {
    ...
    {
        echo "<div class='alert alert-danger m-0 p-0' role='alert'>
        Password lenght should be equal to 8 or greater! 
        </div>";
    }
